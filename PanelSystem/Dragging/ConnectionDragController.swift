...
class ConnectionDragController: NSObject, NSDraggingSource {
    ...
    func trackDrag(forMouseDownEvent mouseDownEvent: NSEvent, in sourceEndpoint: DragEndpoint) {
        ...
        let item = NSDraggingItem(pasteboardWriter: NSPasteboardItem(pasteboardPropertyList: "\(view)", ofType: .string)!)
        
        let session = (sourceEndpoint as! NSView).beginDraggingSession(with: [item], event: mouseDownEvent, source: self)
