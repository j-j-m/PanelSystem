...
private let defaultWindowSize = NSSize(width: NSScreen.main!.frame.width * 0.80, height: NSScreen.main!.frame.height * 0.80)
...
private func basicWindow(with frame: NSRect? = nil, centered: Bool = false) -> NSWindow {
    ...
    let window = NSWindow(contentRect: frame, styleMask: [.closable, .titled, .borderless, .resizable], backing: .buffered, defer: false)
